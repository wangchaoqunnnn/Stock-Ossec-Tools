...
class EastMoneyService(object):
    ...
    def get_watch_list(self):
        """读取预置关注清单并批量获取实时行情（东财失败时回退腾讯）。

        返回 ``(list, source)``。
        """
        try:
            with open(config.WATCH_LIST_PATH, "r", encoding="utf-8") as fp:
                raw = json.load(fp)
        ...
        key = "watch:%s" % ",".join(codes)
        cached = self._cache.get(key)
        if cached is not None:
            return cached

        result = self._watch_eastmoney(raw, codes)
        source = "eastmoney"
        if not result:
            result = self._watch_tencent(raw, codes)
            source = "tencent"

        self._cache.set(key, (result, source))
        return result, source

    def _watch_eastmoney(self, raw, codes):
        ...
        for item in raw:
            ...
            result.append(
                {
                    "code": code,
                    "name": str(row.get("f14") or item.get("name") or ""),
                    ...
                    "industry": str(row.get("f100") or item.get("industry") or ""),
                    "remark": str(item.get("remark") or ""),
                }
            )
        return result

    def _watch_tencent(self, raw, codes):
        ...
        by_code = {}
        for tcode, code in zip(tcodes, codes):
            fields = rows.get(tcode)
            if fields:
                by_code[code] = fields
        ...
        for item in raw:
            ...
            result.append(
                {
                    "code": code,
                    "name": self._clean_name(fields[1]) if len(fields) > 1 else (item.get("name") or ""),
                    "now_price": base["now_price"],
                    "change": base["change"],
                    "change_pct": base["change_pct"],
                    "speed": None,
